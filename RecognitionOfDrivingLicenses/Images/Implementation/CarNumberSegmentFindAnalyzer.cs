...
namespace Bsuir.Misoi.Core.Images.Implementation
{
    public class CarNumberSegmentFindAnalyzer : ISegmentFindAnalyzer
    {
        ...
        public IEnumerable<IFindResult> Find(ISegmentationResult segmantationResult)
        {
            ...
            foreach (var segment in segmantationResult.Segments.Where(s => s.Square > 50))
            {
               ...
               // if ((formFactor > 4.1) && (formFactor < 5.1)) // 520mm X 113mm  form-factor s/p4,64   a/b = 4,6
                if ((formFactor > 1.2) && (formFactor < 1.8)) // 520mm X 113mm  form-factor s/p4,64   a/b = 4,6
                {
                    ...
                    if (lines.Count != 0)
                    {
                        //foreach (var findResult in this.DrawLines(lines, width, height))
                        //{
                        //    yield return findResult;
                        //}
        ...
        private IList<Line> FilterLines(IList<Line> lines)
        {
            ...
            foreach (var line in lines.OrderByDescending(l => l.Lenght))
            {
                result.Clear();
                result.Add(line);

                var parallelLines = lines.Where(l => Math.Abs((line.F - l.F)%180) < _angleDifference)
        ...
        private IFindResult PrepareResult(IList<Line> lines)
        {
            ...
            var parWidth = Math.Abs(first.Distanse(second));
            var parHeight = Math.Abs(second.Distanse(fourth));
            float angle;
            if (parWidth < parHeight)
            {
                angle = 360 - (float)lines[0].F;
                var temp = parWidth;
                parWidth = parHeight;
                parHeight = temp;
            }
            ...
            {
                angle = 90 - (float)lines[0].F;
            }
            var formFactor = parWidth / (double)parHeight;
            if ((formFactor > 1.2) && (formFactor < 1.8) && parWidth > 100) // 520mm X 113mm  form-factor s/p4,64   a/b = 4,6
            {
                return new FindResult(new List<Point> {first, second, fourth, third}, angle);
            }
            else
            {
                return null;
